...
| An intent binds a tool and, optionally, named arguments (a path, a rename's source and destination); it authorizes only proposals whose arguments all match, is consumed only when the policy admits, and can never admit what the policy rejects | Proven (E24b: execution_requires_fitting_intent, intent_never_widens, execution_consumes_the_matched_intent) and tested (tests/intents_resource.py, tests/intent_attacks.py) | Intents bind names, not files (hard links are outside the model); constraint values cannot contain whitespace; the broker orders intents broadest first, tested, not proven |
| A revocation (--revocations: append-only, written by the principal, only read by the broker) is in force for every decision after it is on disk; it never grants; revoking a tool blocks every proposal for it | Proven (E24c: revocation_never_grants, revoking_sole_authority_blocks, revoked_tool_executes_nothing) and tested (tests/revocation.py) | An action already committed (intent consumed, prepared record durable) completes: its effect cannot be recalled, measured. While the broker runs the intents file is the broker's: an outside edit freezes intent-gated actions rather than being applied. A revocation line that cannot be read revokes everything |
...
- **Audit checkpoints.** The broker publishes a signed checkpoint every N records (--checkpoint-every) and at startup and clean shutdown. Fewer than N of the newest records are unprotected at any time; a SIGKILL or a crash publishes nothing, leaving the records since the last checkpoint unprotected (as does a SIGTERM during startup, before the start record, when nothing has yet been served); a failure to publish is recorded in the log, not prevented. Without --checkpoint-sink, an effect-free tail remains detectable only through surviving governed files.
