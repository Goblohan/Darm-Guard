...
| No effect without prior evidence | Tested, not yet proved: an unwritable log means nothing is performed; a tampered log stops the broker from starting; a request with no reply reports unknown (tests/evidence_attacks.py). The lifecycle model (B4) is next |
...
- v0.7.1 (this release) -- evidence before effect: fail-closed prepared records, explicit effect states, startup chain verification, atomic writes. Fixes a 0.7.0 gap in which an effect could occur with no record and the client was told it was rejected.
- Next -- B4, a formal lifecycle model of the broker; a threat model and trusted-computing-base statement; publish third-party comparisons with their maintainers; a kernel-level treatment of paths; finer-grained intents.
