...
- v0.13.0 (this release) -- per-resource intents: an intent names a tool and, optionally, the exact arguments it authorizes (write_file path=/workspace/reports/a.md; a rename's source and destination). Proven in Lean (E24b), including that no intent can widen the policy. Existing tool-only intents files work unchanged.
